...
def train_valid_split(lang_pairs, sample_num_dict, lang_num_dict, train_path, valid_path, seed):
    print("Split data...")
    ...
    src_index = torch.tensor([i for i in range(src_emb_all.size(0))])
    trg_index = torch.tensor([i for i in range(src_emb_all.size(0), src_emb_all.size(0) + trg_emb_all.size(0))])
    ...
    train_src_index, valid_src_index = train_test_split(src_index, test_size=0.1, random_state=seed)
    train_trg_index, valid_trg_index = train_test_split(trg_index, test_size=0.1, random_state=seed)
    ...
    train = {
        ...
        "src_index": train_src_index,
        "trg_index": train_trg_index,
    }
    ...
    valid = {
        ...
        "src_index": valid_src_index,
        "trg_index": valid_trg_index,
    }
    torch.save(valid, valid_path)


def lang_split(train_path, valid_path, langs, lang_num_dict):
    for path in [train_path, valid_path]:
        data = torch.load(path)
        src_emb = data["src_emb"]
        trg_emb = data["trg_emb"]
        src_lang = data["src_lang"]
        trg_lang = data["trg_lang"]
        src_idx = data["src_index"]
        trg_idx = data["trg_index"]

        for lang in langs:
            lang_num = lang_num_dict[lang]
            tmp = {
                "emb": torch.cat((src_emb[src_lang == lang_num], trg_emb[trg_lang == lang_num]), dim=0),
                "index": torch.cat((src_idx[src_lang == lang_num], trg_idx[trg_lang == lang_num]), dim=0),
            }
            torch.save(tmp, path + f".{lang}")
...
def main():
    ...
    train_valid_split(lang_pairs, sample_num_dict, lang_num_dict, args.train_path, args.valid_path, args.seed)
    lang_split(args.train_path, args.valid_path, langs, lang_num_dict)
